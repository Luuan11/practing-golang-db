...
func (m *MemoryRepository) Update(id string, name, category string, count int, price float64) (Product, error) {
	p := Product{
		Name:     name,
		Category: category,
		Count:    count,
...
func (m *MemoryRepository) UpdateName(id string, name string) (Product, error) {
	var p Product
	updated := false
	for i := range ps {
		if ps[i].ID == id {
